  ...
  for (let y = 0; y < draw.canvasHeight; y++) {
    for (let x = 0; x < draw.canvasWidth; x++) {
      ...
      const rr = form(
        ...
        y,
        tick,
        d * Math.sin(tick * 0.01),
        d * Math.cos(tick * 0.01)
      );
      const br = 0;
      const gr = 0;
      ...
      const ry = form(
        ...
        y,
        tick,
        d * Math.sin(tick * 0.01 + 0.1),
        d * Math.cos(tick * 0.01 + 0.1)
      );
      const by = 0;
      const gy = form(
        width,
      ...
      const ba = form(
        ...
        y,
        tick,
        d * Math.sin(tick * 0.01 + 0.2),
        d * Math.cos(tick * 0.01 + 0.2)
      );
      const ga = form(
        width,
        height,
      ...
      const bb = form(
        ...
        y,
        tick,
        d * Math.sin(tick * 0.01 + 0.3),
        d * Math.cos(tick * 0.01 + 0.3)
      );
      const gb = 0;
...
const loop = () => {
  time += 1;
  const loopedTime = 10000 * Math.sin(time / 1000);
  animate(loopedTime);
