...
function fullForm(x, y, tick) {
  ...
  // red
  const rOffsetX = Math.sin(4 * d + tick * 10 + x * 0.1);
  const rOffsetY = Math.cos(4 * d + tick * 10 + y * 0.1);
  ...
  // yellow
  const yOffsetX = Math.sin(4 * d + tick * 10 + 0.1 + x * 0.1);
  const yOffsetY = Math.cos(4 * d + tick * 10 + 0.1 + y * 0.1);
  ...
  // aqua
  const aOffsetX = Math.sin(4 * d + tick * 10 + 0.2 + x * 0.1);
  const aOffsetY = Math.cos(4 * d + tick * 10 + 0.2 + y * 0.1);
  ...
  //blue
  const bOffsetX = Math.sin(4 * d + tick * 10 + 0.3 + x * 0.1);
  const bOffsetY = Math.cos(4 * d + tick * 10 + 0.3 + y * 0.1);
...
function form(width, height, x, y, tick, xOffset, yOffset) {
  ...
  const value = Math.sin(
    (
      tick +
      Math.sqrt(Math.pow(width / 2 - x - xOffset, 2) & Math.pow(height / 2 - y - yOffset, 2))
    ) *
    (5 * (Math.sin(tick * 0.05) + Math.sin(tick * 0.15)))
  );
